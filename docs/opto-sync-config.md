...
`mode` is `client`, `server`, or `hybrid`. Hybrid repositories keep one env-binding inventory and explicit client/server projections. Client code must not require server-only database/NATS credentials, and generated client artifacts must not contain secret values.
...
The checked-in example contains only environment-variable names and safe localhost defaults; database, auth, and credential-bearing transport values remain outside Git.
